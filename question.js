function nazo1(){
    if(document.getElementById('answer1').value=='夢'){
        alert('あたり!');T1();}
else if(document.getElementById('answer1').value=='ゆめ'){
    alert('あたり!');T1();}
else if(document.getElementById('answer1').value=='ユメ'){
    alert('あたり!');T1();}
else if(document.getElementById('answer1').value=='yume'){
    alert('あたり!');T1();}
else if(document.getElementById('answer1').value=='YUME'){
    alert('あたり!');T1();}
else{alert('はずれ!');}
}

function nazo2(){
    if(document.getElementById('answer2').value=='夢'){
        alert('あたり!');T2();}
else if(document.getElementById('answer2').value=='ゆめ'){
    alert('あたり!');T2();}
else if(document.getElementById('answer2').value=='ユメ'){
    alert('あたり!');T2();}
else if(document.getElementById('answer2').value=='yume'){
    alert('あたり!');T2();}
else if(document.getElementById('answer2').value=='YUME'){
    alert('あたり!');T2();}
else{alert('はずれ!');}
}

function nazo3(){
    if(document.getElementById('answer3').value=='夢'){
        alert('あたり!');T3();}
else if(document.getElementById('answer3').value=='ゆめ'){
    alert('あたり!');T3();}
else if(document.getElementById('answer3').value=='ユメ'){
    alert('あたり!');T3();}
else if(document.getElementById('answer3').value=='yume'){
    alert('あたり!');T3();}
else if(document.getElementById('answer3').value=='YUME'){
    alert('あたり!');T3();}
else{alert('はずれ!');}
}

function nazo4(){
    if(document.getElementById('answer4').value=='夢'){
        alert('あたり!');T4();}
else if(document.getElementById('answer4').value=='ゆめ'){
    alert('あたり!');T4();}
else if(document.getElementById('answer4').value=='ユメ'){
    alert('あたり!');T4();}
else if(document.getElementById('answer4').value=='yume'){
    alert('あたり!');T4();}
else if(document.getElementById('answer4').value=='YUME'){
    alert('あたり!');T4();}
else{alert('はずれ!');}
}

function nazo5(){
    if(document.getElementById('answer5').value=='夢'){
        alert('あたり!');T5();}
else if(document.getElementById('answer5').value=='ゆめ'){
    alert('あたり!');T5();}
else if(document.getElementById('answer5').value=='ユメ'){
    alert('あたり!');T5();}
else if(document.getElementById('answer5').value=='yume'){
    alert('あたり!');T5();}
else if(document.getElementById('answer5').value=='YUME'){
    alert('あたり!');T5();}
else{alert('はずれ!');}
}

function nazo6(){
    if(document.getElementById('answer6').value=='夢'){
        alert('あたり!');T6();}
else if(document.getElementById('answer6').value=='ゆめ'){
    alert('あたり!');T6();}
else if(document.getElementById('answer6').value=='ユメ'){
    alert('あたり!');T6();}
else if(document.getElementById('answer6').value=='yume'){
    alert('あたり!');T6();}
else if(document.getElementById('answer6').value=='YUME'){
    alert('あたり!');T6();}
else{alert('はずれ!');}
}

function nazo7(){
    if(document.getElementById('answer7').value=='夢'){
        alert('あたり!');T7();}
else if(document.getElementById('answer7').value=='ゆめ'){
    alert('あたり!');T7();}
else if(document.getElementById('answer7').value=='ユメ'){
    alert('あたり!');T7();}
else if(document.getElementById('answer7').value=='yume'){
    alert('あたり!');T7();}
else if(document.getElementById('answer7').value=='YUME'){
    alert('あたり!');T7();}
else{alert('はずれ!');}
}

function nazo8(){
    if(document.getElementById('answer8').value=='夢'){
        alert('あたり!');T8();}
else if(document.getElementById('answer8').value=='ゆめ'){
    alert('あたり!');T8();}
else if(document.getElementById('answer8').value=='ユメ'){
    alert('あたり!');T8();}
else if(document.getElementById('answer8').value=='yume'){
    alert('あたり!');T8();}
else if(document.getElementById('answer8').value=='YUME'){
    alert('あたり!');T8();}
else{alert('はずれ!');}
}

function nazo9(){
    if(document.getElementById('answer9').value=='夢'){
        alert('あたり!');T9();}
else if(document.getElementById('answer9').value=='ゆめ'){
    alert('あたり!');T9();}
else if(document.getElementById('answer9').value=='ユメ'){
    alert('あたり!');T9();}
else if(document.getElementById('answer9').value=='yume'){
    alert('あたり!');T9();}
else if(document.getElementById('answer9').value=='YUME'){
    alert('あたり!');T9();}
else{alert('はずれ!');}
}

function nazo10(){
    if(document.getElementById('answer10').value=='夢'){
        alert('あたり!');T10();}
else if(document.getElementById('answer10').value=='ゆめ'){
    alert('あたり!');T10();}
else if(document.getElementById('answer10').value=='ユメ'){
    alert('あたり!');T10();}
else if(document.getElementById('answer10').value=='yume'){
    alert('あたり!');T10();}
else if(document.getElementById('answer10').value=='YUME'){
    alert('あたり!');T10();}
else{alert('はずれ!');}
}

function T1(){
    add();
    document.getElementById('question1').innerHTML='Q1.答え:夢';
}

function T2(){
    add();
    document.getElementById('question2').innerHTML='Q2.答え:夢';
}

function T3(){
    add();
    document.getElementById('question3').innerHTML='Q3.答え:夢';
}

function T4(){
    add();
    document.getElementById('question4').innerHTML='Q4.答え:夢';
}

function T5(){
    add();
    document.getElementById('question5').innerHTML='Q5.答え:夢';
}

function T6(){
    add();
    document.getElementById('question6').innerHTML='Q6.答え:夢';
}

function T7(){
    add();
    document.getElementById('question7').innerHTML='Q7.答え:夢';
}

function T8(){
    add();
    document.getElementById('question8').innerHTML='Q8.答え:夢';
}

function T9(){
    add();
    document.getElementById('question9').innerHTML='Q9.答え:夢';
}

function T10(){
    add();
    document.getElementById('question10').innerHTML='Q10.答え:夢';
}